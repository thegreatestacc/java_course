...
- **Production** - основное окружение (деплоится из ветки `main`)
- **Dev** - тестовое окружение для проверки новых фич (деплоится из ветки `dev`)

Подробная инструкция по настройке dev окружения: [DEV_ENVIRONMENT_SETUP.md](./DEV_ENVIRONMENT_SETUP.md)
...
4. Обновит образы и перезапустит контейнеры в production

### Dev окружение

При каждом push в ветку `dev`, `develop` или `development` GitHub Actions автоматически:
1. Соберет Docker образы с тегом `dev`
2. Загрузит их в GitHub Container Registry
3. Подключится к серверу по SSH
4. Обновит образы и перезапустит контейнеры в dev окружении

**Важно:** Dev окружение использует отдельную базу данных и порты, поэтому не влияет на production.

Подробнее: [DEV_ENVIRONMENT_SETUP.md](./DEV_ENVIRONMENT_SETUP.md)
